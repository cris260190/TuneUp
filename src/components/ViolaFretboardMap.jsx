...
// Viola: B F C G — in fifths, a fifth lower than violin, a fifth higher than cello
const OPEN   = [11, 5, 0, 7]
const FREQS  = [123.47, 174.61, 261.63, 392.00]   // B2 F3 C4 G4
const LABELS = ['B', 'F', 'C', 'G']
...
export default function ViolaFretboardMap({ onNoteClick }) {
        ...
        >
          <style>{`
            ...
            .violaNoteRing {
              ...
              animation: violaNoteRing 0.9s ease-out forwards;
            }
          `}</style>
          ...
          {/* fret lines — dashed because viola is fretless */}
          {Array.from({ length: NUM_FRETS }, (_, i) => (
            <line key={i + 1}
          ...
          {/* strings — B string thickest, G thinnest */}
          {[0, 1, 2, 3].map(i => (
            <line key={i}
