...
import { VIOLA_CHORDS, VIOLA_CHORD_ROOTS, VIOLA_CHORD_TYPES } from '../data/violaChordData'
...
export default function ViolaChordLibraryPage() {
  const navigate = useNavigate()
  const { theme, toggleTheme } = useTheme()
  const { t } = useLanguage()
  useSEO(SEO.chordsViola)

  const [selectedRoot, setSelectedRoot] = useState('C')
  const [selectedType, setSelectedType] = useState('All')

  const availableTypes = ['All', ...VIOLA_CHORD_TYPES.filter(tp =>
    VIOLA_CHORDS.some(c => c.root === selectedRoot && c.type === tp)
  )]

  const filtered = VIOLA_CHORDS.filter(c => {
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '3rem 1.5rem', width: '100%' }}>
        ...
        <p style={{ color: 'var(--muted2)', fontSize: '.9rem', marginBottom: '1.8rem' }}>
          Positional chord shapes for viola — tuning B F C G. Click a chord for full detail.
        </p>

        {/* Chords | Fretboard Map tab strip */}
        <div style={{ display: 'flex', gap: '.4rem', marginBottom: '1.8rem' }}>
          <button style={TAB(true)}>Chords</button>
          <button onClick={() => navigate('/chords/viola/fretboard')} style={TAB(false)}>Fretboard Map</button>
        </div>

        {/* Root tabs */}
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.4rem', marginBottom: '1.2rem' }}>
          {VIOLA_CHORD_ROOTS.map(root => {
            const hasChords = VIOLA_CHORDS.some(c => c.root === root)
        ...
        {filtered.length === 0 ? (
          ...
          <div style={{
            ...
            {filtered.map(chord => (
              <div
                key={chord.name}
                onClick={() => navigate(`/chords/viola/${encodeURIComponent(chord.name.toLowerCase())}`)}
                style={{
