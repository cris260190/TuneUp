...
const STRING_LABELS = ['B', 'F', 'C', 'G']

export default function ViolaChordPage() {
  const navigate = useNavigate()
  const { chord: slug } = useParams()
  const { theme, toggleTheme } = useTheme()
  const { t } = useLanguage()

  const chord = getViolaChordBySlug(slug || '')

  useSEO(chord ? {
    title: `${chord.name} Viola Chord — How to Play ${chord.fullName} | TuneUp`,
    description: `Free ${chord.name} viola chord diagram with finger positions. ${chord.fullName} on viola tuning B F C G — no app needed.`,
    url: `https://freetuner.app/chords/viola/${chord.name.toLowerCase()}`,
  } : { title: 'Viola Chord Not Found | TuneUp', description: '', url: '' })
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ maxWidth: '560px', margin: '0 auto', padding: '3.5rem 1.5rem', width: '100%', textAlign: 'center' }}>
        ...
        <p style={{ color: 'var(--muted2)', fontSize: '.85rem', marginBottom: '2rem' }}>
          {chord.fullName} — {t?.chordHowToPlay || 'How to play'} · B F C G
        </p>
