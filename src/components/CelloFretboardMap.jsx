...
// Cello: C G D A — in fifths, like violin but a fifth lower
const OPEN   = [0, 7, 2, 9]
const FREQS  = [65.41, 98.00, 146.83, 220.00]   // C2 G2 D3 A3
const LABELS = ['C', 'G', 'D', 'A']
...
export default function CelloFretboardMap({ onNoteClick }) {
        ...
        >
          <style>{`
            ...
            .celloNoteRing {
              ...
              animation: celloNoteRing 0.9s ease-out forwards;
            }
          `}</style>
          ...
          {/* fret lines (dashed — cello is fretless; these are approximate positions) */}
          {Array.from({ length: NUM_FRETS }, (_, i) => (
            <line key={i + 1}
          ...
          {/* strings — C string thickest, A thinnest */}
          {[0, 1, 2, 3].map(i => (
            <line key={i}
              x1={LEFT} y1={stringY(i)} x2={LEFT + NUM_FRETS * FRET_W} y2={stringY(i)}
              stroke="var(--border)" strokeWidth={3 - i * 0.6} />
          ))}
