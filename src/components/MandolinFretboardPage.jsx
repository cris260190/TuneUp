...
import MandolinFretboardMap from './MandolinFretboardMap'
...
function playNote(freq) {
  ...
  // Low damping = bright metallic attack, short decay — characteristic of mandolin
  const damping = 0.993
...
export default function MandolinFretboardPage() {
  const navigate = useNavigate()
  const { theme, toggleTheme } = useTheme()
  const { t } = useLanguage()
  useSEO(SEO.chordsMandolinFretboard)
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '3rem 1.5rem', width: '100%' }}>
        ...
        <div style={{ display: 'flex', gap: '.4rem', marginBottom: '2rem' }}>
          <button onClick={() => navigate('/chords/mandolin')} style={TAB(false)}>Chords</button>
        ...
        <p style={{ color: 'var(--muted2)', fontSize: '.9rem', marginBottom: '1.8rem' }}>
          Every note on the mandolin neck, colour-coded across all 4 courses and 12 frets.
          Tap a note to hear it.
        </p>

        <MandolinFretboardMap onNoteClick={playNote} />

        <div style={{ display: 'flex', gap: '.75rem', flexWrap: 'wrap', marginTop: '3rem' }}>
          <button
            onClick={() => navigate('/chords/mandolin')}
            style={{ ...pillBtn, fontSize: '1rem', padding: '0.5rem 1.4rem' }}
          >
            ← Mandolin Chord Library
          </button>
          <button
            onClick={() => navigate('/strings/Mandolin')}
            style={{ ...pillBtn, fontSize: '1rem', padding: '0.5rem 1.4rem' }}
          >
