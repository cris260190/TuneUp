...
import { getCelloChordBySlug } from '../data/celloChordData'
...
const STRING_LABELS = ['C', 'G', 'D', 'A']

export default function CelloChordPage() {
  const navigate = useNavigate()
  const { chord: slug } = useParams()
  const { theme, toggleTheme } = useTheme()
  const { t } = useLanguage()

  const chord = getCelloChordBySlug(slug || '')

  useSEO(chord ? {
    title: `${chord.name} Cello Chord — How to Play ${chord.fullName} | TuneUp`,
    description: `Free ${chord.name} cello chord diagram with finger positions. ${chord.fullName} on standard cello tuning C G D A — no app needed.`,
    url: `https://freetuner.app/chords/cello/${chord.name.toLowerCase()}`,
  } : { title: 'Cello Chord Not Found | TuneUp', description: '', url: '' })

  if (!chord) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <button onClick={() => navigate('/chords/cello')} style={pillBtn}>← All Cello Chords</button>
      </div>
    )
  }
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ maxWidth: '560px', margin: '0 auto', padding: '3.5rem 1.5rem', width: '100%', textAlign: 'center' }}>
        ...
        <p style={{ color: 'var(--muted2)', fontSize: '.85rem', marginBottom: '2rem' }}>
          {chord.fullName} — {t?.chordHowToPlay || 'How to play'} · C G D A
        </p>
        ...
        <div style={{ display: 'flex', gap: '.75rem', justifyContent: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => navigate('/chords/cello')} style={{ ...pillBtn, fontSize: '1rem', padding: '0.5rem 1.4rem' }}>
            ← All Cello Chords
          </button>
          <button onClick={() => navigate('/strings/Cello')} style={{ ...pillBtn, fontSize: '1rem', padding: '0.5rem 1.4rem' }}>
