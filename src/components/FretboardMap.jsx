...
const OPEN      = [4, 9, 2, 7, 11, 4]          // semitone index: E A D G B e
// Open-string frequencies in standard tuning (Hz)
...
function noteFreq(si, fi) { return OPEN_FREQS[si] * Math.pow(2, fi / 12) }

export default function FretboardMap({ onNoteClick }) {
  const [playingKey, setPlayingKey] = useState(null)

  function handleClick(si, fi) {
    if (!onNoteClick) return
    const key = `${si}-${fi}`
    onNoteClick(noteFreq(si, fi))
    setPlayingKey(key)
    setTimeout(() => setPlayingKey(k => k === key ? null : k), 900)
  }
  ...
  return (
    <div>
        ...
        >
          <style>{`
            ...
            .noteRing {
              ...
              animation: noteRing 0.85s ease-out forwards;
            }
          `}</style>
          ...
          {Array.from({ length: NUM_FRETS }, (_, i) => (
            <text key={i + 1} x={noteX(i + 1)} y={TOP - 12}
              textAnchor="middle" fontSize="9"
              fontFamily="'Space Mono', monospace" fill="var(--muted2)">{i + 1}</text>
          ))}

          {/* nut */}
          <rect x={LEFT - 3} y={TOP - 2} width={4} height={5 * STRING_H + 4} fill="var(--gold)" />

          {/* fret lines */}
          {Array.from({ length: NUM_FRETS }, (_, i) => (
            <line key={i + 1}
              x1={fretLineX(i + 1)} y1={TOP} x2={fretLineX(i + 1)} y2={TOP + 5 * STRING_H}
          ...
          {Array.from({ length: 6 }, (_, i) => (
            <line key={i}
              ...
              stroke="var(--border)" strokeWidth={1 + (5 - i) * 0.2} />
          ))}

          {/* string labels */}
          {STRING_LABELS.map((lbl, i) => (
            <text key={i} x={LEFT - 30} y={stringY(i) + 4}
          ...
          {[3, 5, 7, 9].map(f => (
            <circle key={f} cx={noteX(f)} cy={TOP + 5 * STRING_H + 15} r={4} fill="var(--border)" />
          ))}
          ...
          {OPEN.flatMap((openNote, si) =>
            Array.from({ length: NUM_FRETS + 1 }, (_, fi) => {
              ...
              return (
                <g
                  key={key}
                  onClick={() => handleClick(si, fi)}
                  style={{ cursor: clickable ? 'pointer' : 'default' }}
                >
                  {/* expanding ring on play */}
                  {playingKey === key && (
      ...
      {/* color legend */}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 14px', marginTop: '18px', justifyContent: 'center' }}>
        {NOTE_NAMES.map((note, i) => (
          <div key={note} style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
            <div style={{ width: 13, height: 13, borderRadius: '50%', background: NOTE_COLORS[i], flexShrink: 0 }} />
            <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '.65rem', color: 'var(--text)' }}>{note}</span>
          </div>
        ))}
      </div>
    </div>
