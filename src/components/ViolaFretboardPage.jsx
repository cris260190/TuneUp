...
import ViolaFretboardMap from './ViolaFretboardMap'
...
export default function ViolaFretboardPage() {
  const navigate = useNavigate()
  const { theme, toggleTheme } = useTheme()
  const { t } = useLanguage()
  useSEO(SEO.chordsViolaFretboard)
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '3rem 1.5rem', width: '100%' }}>
        <div style={{ display: 'flex', gap: '.4rem', marginBottom: '2rem' }}>
          <button onClick={() => navigate('/chords/viola')} style={TAB(false)}>Chords</button>
        ...
        <p style={{ color: 'var(--muted2)', fontSize: '.9rem', marginBottom: '.6rem' }}>
          Every note across 4 strings and 12 positions — tuning B F C G. Tap a note to hear it.
        </p>
        ...
        <div style={{
          ...
          ∿ Approximate positions — viola is a fretless instrument
        </div>

        <ViolaFretboardMap onNoteClick={playNote} />
