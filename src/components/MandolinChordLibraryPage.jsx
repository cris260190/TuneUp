...
import { MANDOLIN_CHORDS, MANDOLIN_CHORD_ROOTS, MANDOLIN_CHORD_TYPES } from '../data/mandolinChordData'
...
export default function MandolinChordLibraryPage() {
  const navigate = useNavigate()
  const { theme, toggleTheme } = useTheme()
  const { t } = useLanguage()
  useSEO(SEO.chordsMandolin)

  const [selectedRoot, setSelectedRoot] = useState('C')
  const [selectedType, setSelectedType] = useState('All')

  const availableTypes = ['All', ...MANDOLIN_CHORD_TYPES.filter(tp =>
    MANDOLIN_CHORDS.some(c => c.root === selectedRoot && c.type === tp)
  )]

  const filtered = MANDOLIN_CHORDS.filter(c => {
  ...
  function handleChordClick(chord) {
    navigate(`/chords/mandolin/${encodeURIComponent(chord.name.toLowerCase())}`)
  }
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '3rem 1.5rem', width: '100%' }}>
        ...
        <p style={{ color: 'var(--muted2)', fontSize: '.9rem', marginBottom: '1.8rem' }}>
          Chord diagrams for mandolin — standard tuning G D A E. Click a chord for full detail.
        </p>

        {/* Chords | Fretboard Map tab strip */}
        <div style={{ display: 'flex', gap: '.4rem', marginBottom: '1.8rem' }}>
          <button style={TAB(true)}>Chords</button>
          <button onClick={() => navigate('/chords/mandolin/fretboard')} style={TAB(false)}>Fretboard Map</button>
        </div>

        {/* Root tabs */}
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.4rem', marginBottom: '1.2rem' }}>
          {MANDOLIN_CHORD_ROOTS.map(root => {
            const hasChords = MANDOLIN_CHORDS.some(c => c.root === root)
