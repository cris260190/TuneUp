...
// Violin: G D A E — same intervals as mandolin, an octave lower
const OPEN   = [7, 2, 9, 4]
...
export default function ViolinFretboardMap({ onNoteClick }) {
  ...
  function handleClick(si, fi) {
    ...
    onNoteClick(freq)
    setPlayingKey(key)
    setTimeout(() => setPlayingKey(k => k === key ? null : k), 800)
  }
        ...
        >
          <style>{`
            ...
            .violinNoteRing {
              ...
              animation: violinNoteRing 0.8s ease-out forwards;
            }
          `}</style>
          ...
          {/* dashed fret lines — violin is fretless */}
          {Array.from({ length: NUM_FRETS }, (_, i) => (
            <line key={i + 1}
              ...
              strokeDasharray={i + 1 === 12 ? 'none' : '3 3'}
            />
          ))}

          {/* strings — G thickest, E thinnest */}
          {[0, 1, 2, 3].map(i => (
            <line key={i}
              x1={LEFT} y1={stringY(i)} x2={LEFT + NUM_FRETS * FRET_W} y2={stringY(i)}
              stroke="var(--border)" strokeWidth={2.5 - i * 0.5} />
          ))}
