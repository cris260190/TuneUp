...
import { VIOLIN_CHORDS, VIOLIN_CHORD_ROOTS, VIOLIN_CHORD_TYPES } from '../data/violinChordData'
...
export default function ViolinChordLibraryPage() {
  const navigate = useNavigate()
  const { theme, toggleTheme } = useTheme()
  const { t } = useLanguage()
  useSEO(SEO.chordsViolin)

  const [selectedRoot, setSelectedRoot] = useState('C')
  const [selectedType, setSelectedType] = useState('All')

  const availableTypes = ['All', ...VIOLIN_CHORD_TYPES.filter(tp =>
    VIOLIN_CHORDS.some(c => c.root === selectedRoot && c.type === tp)
  )]

  const filtered = VIOLIN_CHORDS.filter(c => {
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '3rem 1.5rem', width: '100%' }}>
        ...
        <p style={{ color: 'var(--muted2)', fontSize: '.9rem', marginBottom: '1.8rem' }}>
          Positional chord shapes for violin — tuning G D A E. Click a chord for full detail.
        </p>

        <div style={{ display: 'flex', gap: '.4rem', marginBottom: '1.8rem' }}>
          <button style={TAB(true)}>Chords</button>
          <button onClick={() => navigate('/chords/violin/fretboard')} style={TAB(false)}>Fretboard Map</button>
        </div>

        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.4rem', marginBottom: '1.2rem' }}>
          {VIOLIN_CHORD_ROOTS.map(root => {
            const hasChords = VIOLIN_CHORDS.some(c => c.root === root)
        ...
        {filtered.length === 0 ? (
          ...
          <div style={{
            ...
            {filtered.map(chord => (
              <div
                key={chord.name}
                onClick={() => navigate(`/chords/violin/${encodeURIComponent(chord.name.toLowerCase())}`)}
                style={{
