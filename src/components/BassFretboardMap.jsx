...
export default function BassFretboardMap({ numStrings = 4, onNoteClick }) {
  const [playingKey, setPlayingKey] = useState(null)
  const tuning = TUNINGS[numStrings] || TUNINGS[4]
  const SVG_H  = TOP + (numStrings - 1) * STRING_H + 28

  function handleClick(si, fi) {
    if (!onNoteClick) return
    const freq = tuning.freqs[si] * Math.pow(2, fi / 12)
    const key  = `${si}-${fi}`
    onNoteClick(freq)
    setPlayingKey(key)
  ...
  // Low strings are thicker (bass strings are visually heavier than guitar)
  function stringThickness(i) {
    const max = numStrings === 5 ? 3.5 : 3
    return max - i * 0.5
  }
        ...
        >
          <style>{`
            ...
            .bassNoteRing {
              ...
              animation: bassNoteRing 0.85s ease-out forwards;
            }
          `}</style>

          {/* position labels + tick marks */}
          ...
          {/* nut */}
          <rect x={LEFT - 3} y={TOP - 2} width={4} height={(numStrings - 1) * STRING_H + 4}
            fill="var(--gold)" />
          ...
          {Array.from({ length: NUM_FRETS }, (_, i) => (
            <line key={i + 1}
              ...
              x2={fretLineX(i + 1)} y2={TOP + (numStrings - 1) * STRING_H}
              stroke="var(--border)" strokeWidth={i + 1 === 12 ? 2.5 : 1} />
          ))}

          {/* strings — heavier gauge = thicker line */}
          {Array.from({ length: numStrings }, (_, i) => (
            <line key={i}
              x1={LEFT} y1={stringY(i)} x2={LEFT + NUM_FRETS * FRET_W} y2={stringY(i)}
              stroke="var(--border)" strokeWidth={stringThickness(i)} />
          ))}

          {/* string labels */}
          {tuning.labels.map((lbl, i) => (
            <text key={i} x={LEFT - 30} y={stringY(i) + 4} textAnchor="middle"
              fontSize="11" fontWeight="600"
              fontFamily="'Space Mono', monospace" fill="var(--muted2)">{lbl}</text>
          ))}

          {/* position marker dots below the strings */}
          {[3, 5, 7, 9].map(f => (
            <circle key={f}
              cx={noteX(f)} cy={TOP + (numStrings - 1) * STRING_H + 15}
              r={4} fill="var(--border)" />
          ))}
          {/* double dot at XII */}
          <circle cx={noteX(12) - 8} cy={TOP + (numStrings - 1) * STRING_H + 15} r={4} fill="var(--border)" />
          <circle cx={noteX(12) + 8} cy={TOP + (numStrings - 1) * STRING_H + 15} r={4} fill="var(--border)" />

          {/* note circles */}
          {tuning.open.flatMap((openNote, si) =>
