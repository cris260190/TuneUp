...
import { BASS4_CHORDS, BASS5_CHORDS, BASS_CHORD_ROOTS, BASS_CHORD_TYPES } from '../data/bassChordData'
...
export default function BassChordLibraryPage() {
  ...
  const chords = strCount === 4 ? BASS4_CHORDS : BASS5_CHORDS

  const availableTypes = ['All', ...BASS_CHORD_TYPES.filter(tp =>
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '3rem 1.5rem', width: '100%' }}>
        ...
        <div style={{ display: 'flex', gap: '.4rem', marginBottom: '1.8rem' }}>
          ...
          <button onClick={() => navigate('/chords/bass/fretboard')} style={TAB(false)}>Fretboard Map</button>
        </div>

        {/* 4-string / 5-string tabs */}
        <div style={{ display: 'flex', gap: '.4rem', marginBottom: '1.8rem' }}>
          <button onClick={() => { setStrCount(4); setSelectedType('All') }} style={TAB(strCount === 4)}>
            4-String
          </button>
          <button onClick={() => { setStrCount(5); setSelectedType('All') }} style={TAB(strCount === 5)}>
            5-String
          </button>
        </div>
        ...
        {filtered.length === 0 ? (
          <div style={{ color: 'var(--muted2)', fontSize: '.9rem', padding: '2rem 0' }}>
            No chords yet for {selectedRoot} {selectedType === 'All' ? '' : selectedType}.
          </div>
        ) : (
          <div style={{
            display: 'grid',
            ...
            {filtered.map(chord => (
              ...
              >
                <ChordDiagram chord={chord} size={90} numStrings={strCount} />
                <div style={{
                  fontFamily: "'Cormorant Garamond', serif",
