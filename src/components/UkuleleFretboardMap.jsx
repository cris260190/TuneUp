...
const TUNINGS = {
  ...
  baritone: {
    open:   [2, 7, 11, 4],
    freqs:  [146.83, 196.00, 246.94, 329.63],
    labels: ['D', 'G', 'B', 'E'],
  },
}
...
const SVG_H    = TOP + 3 * STRING_H + 28   // always 4 strings → 3 gaps
...
export default function UkuleleFretboardMap({ tuning: tuningKey = 'standard', onNoteClick }) {
  ...
  function handleClick(si, fi) {
    ...
    onNoteClick(freq)
    setPlayingKey(key)
    setTimeout(() => setPlayingKey(k => k === key ? null : k), 700)
  }
        ...
        >
          <style>{`
            ...
            .ukeNoteRing {
              ...
              animation: ukeNoteRing 0.7s ease-out forwards;
            }
          `}</style>
          ...
          {/* strings — uniform gauge (ukulele strings are similar in thickness) */}
          {[0, 1, 2, 3].map(i => (
            <line key={i}
