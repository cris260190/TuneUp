...
import { BANJO_CHORDS, BANJO_CHORD_ROOTS, BANJO_CHORD_TYPES } from '../data/banjoChordData'
...
export default function BanjoChordLibraryPage() {
  const navigate = useNavigate()
  const { theme, toggleTheme } = useTheme()
  const { t } = useLanguage()
  useSEO(SEO.chordsBanjo)

  const [selectedRoot, setSelectedRoot] = useState('C')
  const [selectedType, setSelectedType] = useState('All')

  const availableTypes = ['All', ...BANJO_CHORD_TYPES.filter(tp =>
    BANJO_CHORDS.some(c => c.root === selectedRoot && c.type === tp)
  )]

  const filtered = BANJO_CHORDS.filter(c => {
  ...
  function handleChordClick(chord) {
    navigate(`/chords/banjo/${encodeURIComponent(chord.name.toLowerCase())}`)
  }
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '3rem 1.5rem', width: '100%' }}>
        ...
        <p style={{ color: 'var(--muted2)', fontSize: '.9rem', marginBottom: '1.8rem' }}>
          Chord diagrams for 5-string banjo — Open G tuning (g D G B d). Click a chord for full detail.
        </p>

        {/* Root tabs */}
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.4rem', marginBottom: '1.2rem' }}>
          {BANJO_CHORD_ROOTS.map(root => {
            const hasChords = BANJO_CHORDS.some(c => c.root === root)
        ...
        <button onClick={() => navigate('/strings/Banjo 5-str')} style={{
